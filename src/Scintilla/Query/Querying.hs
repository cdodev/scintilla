...
module Scintilla.Query.Querying where
...
instance {-# OVERLAPPABLE #-} (Monoid (KeyList kvs'), IdentityResultF kvs') => Somewhere ('KV k v ': kvs') ('KV k v ': kvs') ('KV k v) where
  somewhere _ = Here

instance {-# OVERLAPPABLE #-} Somewhere kvs kvs' kv => Somewhere kvs ('KV k v ': kvs') kv where
...
runQuerying (Querying Nil (Compose m)) = fmap (\(ResultConst a) -> a) m
